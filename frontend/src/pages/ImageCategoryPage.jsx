...
import { listImageCategories, createImageCategory, updateImageCategory, deleteImageCategory } from '../services/imageCategory';
...
export default function ImageCategoryPage() {
  ...
  const loadCategories = async () => {
    ...
    try {
      ...
      if (resp.code === 0 && resp.data) {
        ...
        throw new Error(resp.message || '加载图片分类失败');
      }
    } catch (err) {
      console.error('加载图片分类失败:', err);
      showError(err.message || '加载图片分类失败');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCreate = async (e) => {
    ...
    try {
      const resp = await createImageCategory(formData);
      if (resp.code === 0) {
        success('创建成功');
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      const resp = await updateImageCategory({ id: currentCategory.id, ...formData });
      if (resp.code === 0) {
        success('更新成功');
  ...
  const handleDelete = async (id) => {
    const confirmed = await confirm('确定要删除这个图片分类吗？', '确认删除');
    if (!confirmed) return;

    try {
      const resp = await deleteImageCategory(id);
      if (resp.code === 0) {
        success('删除成功');
        loadCategories();
      } else {
